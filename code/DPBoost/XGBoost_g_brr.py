    ...
    def _calc_node_val(self, x, y, exp_leaf, shrinkage, lap1_rate, n_trees, cur_tree):
        ...
        for i in range(x.shape[0]):
            if abs(x.iloc[i, -1]) > c:
                ...
                a= abs(x.iloc[i,-1]) % gap
                if x.iloc[i,-1] > 0:
                    if a * 2 >= gap:
                        x.iloc[i,-1] = x.iloc[i, -1] - a + gap
                    else:
                        x.iloc[i, -1] = x.iloc[i,-1] -a
                else:
                    if a * 2 >= gap:
                        x.iloc[i, -1] = x.iloc[i, -1] + a - gap
                    else:
        ...
        def assign_weights(sorted_similarity):
            weights = []

            for row in sorted_similarity:
                row_weights = [np.exp(exp_leaf)] + [1] * (len(row) - 1)
        ...
        def calculate_item_probabilities(c, gap, n_divide, true_item):
            ...
            weights = assign_weights(sorted_similarity)
            ...
            closest_item_index = min(range(len(items)), key=lambda i: abs(items[i] - true_item))
            closest_sequence_probability = sequence_probabilities[closest_item_index]

            return closest_sequence_probability

        def randomly_select_value(probability_dict):
            items, probs = zip(*probability_dict.items())
            selected_value = np.random.choice(items, p=probs)
            return selected_value
        for i in range(x.shape[0]):
            true_item = x.iloc[i, -1]
            result = calculate_item_probabilities(c, gap, n_divide,true_item)
            chosen_item= randomly_select_value(result)
            x.iloc[i, -1] = chosen_item
        gi = x.loc[:, 'grad'].sum()
        hi = x.shape[0]
...
class xgbRegression(Tree):
    ...
    def fit(self, X, y, exp, internal_rate, lap1_rate):
        ...
        print('\033[0;32mXGB 隐私预算为 {} 时训练 {} 颗树删除样本比例为：{}\033[0m'.format(exp, self._n_estimator, len(delete_index_list)/n_samples))

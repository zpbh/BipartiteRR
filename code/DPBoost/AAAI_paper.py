...
class Tree(object):
            ...
            for i in range(n_feature):
                ...
                for fea_val in feature_value:
                    ...
                    if X_left.shape[0] > 0 and y_left.shape[0] > 0 and X_right.shape[0] > 0 and y_right.shape[0] > 0:
                        ...
                        #     gain = after_divide_left + after_divide_right
                        #     # gain = (after_divide_left + after_divide_right - before_divide) / 2 # 换成自己的分裂函数看效果
    ...
    def _calc_node_val(self, x, exp_leaf, shrinkage, t_node, y=None):
        gi = x.loc[:, 'grad'].sum()
        hi = x.shape[0]
        vt = - gi / (hi + self.reg_lambda)               # 原本叶节点值

        # 裁剪
        threshold = np.power(1 - shrinkage, t_node - 1)  # 阈值
        if vt == 0:                                      # 除 0 错误
            leaf_value = 0
        else:
...
class xgbRegression(Tree):
    def __init__(self, n_estimator=50, trees_in_ensemble=50, max_depth=6, reg_lambda=0.1, learning_rate=0.1, min_sample=2):
        ...
        self._n_estimator = n_estimator # 树的总量
    ...
    def fit(self, X, y, exp):
        ...
        for i in range(self._n_estimator):
            ...
            #     x_gi.iloc[v, 0] = predict_value[v] - y_copy.iloc[v, 0]

        # print('\033[0;32mAAAI 隐私预算为 {} 时训练 {} 颗树平均删除样本比例为：{}\033[0m'.format(exp, self._n_estimator, len(delete_index_list)/n_samples))
